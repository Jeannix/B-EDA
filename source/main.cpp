...
void run_test() {
  ...
  auto *threads = new std::thread[NUMBER_THREADS];
...
void run_parallel_asynchronous_test() {
  ...
  auto *insert_threads = new std::thread[NUMBER_THREADS];
  auto *search_threads = new std::thread[NUMBER_THREADS];
...
[[noreturn]] void search_when_notified(EDA::Concurrent::BLinkTree<3, int> *b_link, int id) {
  while (true) {
    std::unique_lock<std::mutex> lock(MUTEX[id]);
